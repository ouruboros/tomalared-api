  ...
  # @user_session = UserSession.new    
    ...
    if params[:accept]      
      ...
      if @user.save_without_session_maintenance 
        ...
        render :json => {:message => "Cuenta creada!!"}
      else
        render :json => {:message => "Hubo un problema creando tu usuario."} #, :status => :bad_request
      end
    else
      render :json => {:message => "Debe aceptar las condiciones."} #, :status => :bad_request
    end
  end
